...
from mylib.BS_formulas import bs_price, bs_delta, bs_IV
...
def construct_tracer(df, df_rate, cp_flag, target_tau_days, ALLOWED_ADJ_FACTOR_CHANGE=[0.99, 1.01]):
    ...
    df_select = df[(df['cp_flag'] == cp_flag) ]  # & df['IV0'].notnull()
    ...
    for d, dfg in df_select.groupby('date'):
        ...
        # fill in IV0 if missing
        if np.isnan(lower_right['IV0']):
            lower_right['IV0'] = bs_IV(V0=lower_right['V0'], S0=lower_right['S0'], K=lower_right['K'], tau=lower_right['tau'], r=lower_right['r'], cp_flag=cp_flag)
        if np.isnan(lower_left['IV0']):
            lower_left['IV0'] = bs_IV(V0=lower_left['V0'], S0=lower_left['S0'], K=lower_left['K'], tau=lower_left['tau'], r=lower_left['r'], cp_flag=cp_flag)
        if np.isnan(upper_right['IV0']):
            upper_right['IV0'] = bs_IV(V0=upper_right['V0'], S0=upper_right['S0'], K=upper_right['K'], tau=upper_right['tau'], r=upper_right['r'], cp_flag=cp_flag)
        if np.isnan(upper_left['IV0']):
            upper_left['IV0'] = bs_IV(V0=upper_left['V0'], S0=upper_left['S0'], K=upper_left['K'], tau=upper_left['tau'], r=upper_left['r'], cp_flag=cp_flag)
        
        IV0, K, IV1, S1, tau1, r1, V0 = [], [], [], [], [], [], []
    ...
    df_out = pd.DataFrame({'date': d_list, 'S0': S0_list, 'S1': S1_list, 'IV0': IV_interp_list, 'V0_interp': V0_interp_list})
    df_out['K'] = df_out['S0']
